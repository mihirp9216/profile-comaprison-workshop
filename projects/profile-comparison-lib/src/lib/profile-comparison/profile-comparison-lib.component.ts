...
//   }
// }
/////////////////Lastly working Updated code start. /////////////////////////////
import {
  Component,
  Input,
  Output,
  ...
  implements OnChanges, AfterViewInit, OnInit
{
  ...
  /** Returns CSS style for face bounding box overlay */
  getFaceBoxStyle(box: FaceBoundingBox) {
    return {
      top: `${(box.y / 260) * 100}%`,
      left: `${(box.x / 200) * 100}%`,
      ...
      'border-radius': '6px',
      'box-shadow': '0 0 10px rgba(75,156,181,0.75)',
    };
  }

  /** Returns object-fit position to center face aligned */
  getObjectPosition(box: FaceBoundingBox | null) {
    if (!box) return 'center center';
    const centerY = box.y + box.height * 0.4;
    const posY = Math.min(Math.max(((130 - centerY) / 260) * 100, 0), 100);
  ...
  /** Return all unique interests from combined users, used to render similarity matrix headers */
  getAllUniqueInterests(): string[] {
    return Array.from(
      new Set([
        ...this.orderedInterestsUser1,
  ...
  /** Return CSS background color style for interest pill based on average similarity */
  getInterestColor(interest: string) {
    const allInterests = this.getAllUniqueInterests();
    const idx = allInterests.indexOf(interest);
    if (idx === -1 || !this.similarityMatrix.length) {
    ...
    const avgSimilarity =
      this.similarityMatrix[idx].reduce((acc, v) => acc + v, 0) /
      this.similarityMatrix.length;
    const tealHue = 197; // Bright blue tone
    const lightness = 80 - avgSimilarity * 40;
    return {
      'background-color': `hsl(${tealHue}, 85%, ${lightness}%)`,
      color: avgSimilarity > 0.55 ? '#fff' : '#004E64',
      'font-weight': '700',
      cursor: 'default',
      padding: '6px 16px',
      'border-radius': '22px',
  ...
  /** Return heatmap color for similarity matrix cell (0-1) */
  getHeatmapColor(value: number): string {
    if (value === 1) return '#0366d6'; // bright blue
    if (value === 0) return '#e8f1fb'; // very light blue

    // gradient from light blue to bright blue
    const startRgb = [232, 241, 251];
    const endRgb = [3, 102, 214];
